            ...
            container.pagination({
                ...
                callback: function(data, pagination){
                    ...
                    $.each(data, function (index, item){
                        ...
                        <td class="col-1" onclick="get_question_detail(${item.id},${item.answer})"> ✅ </td>
...
async function get_question_detail(q_id,done_code){
    await $.ajax({
       type: "GET",
       url: "/teacher/question_detail/"+q_id,
       data: {},
       success: function (response) {
        category = q_category(response["category"])
        ...
        if(category == '일반문의'){
            ...
            if(category == '이반문의'){code = 2}else{code=3}
