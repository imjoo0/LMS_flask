...
function plusconsulting_history(student_id, b_id, t_id) {
    const student_info = myStudentData.filter(a=>a.student_id == student_id)[0]
...
async function get_consulting_student(done_code) {
    ...
    if (consulting_targetdata.length == 0) {
        ...
        <thead>
            <tr class="row">
                <th class="col-2" onclick="${sort_consultingoption('ban_desc',done_code)}" id="ban_name_sort">반 이름순 정렬👉</th>
                <th class="col-6" onclick="${sort_consultingoption('name_desc',done_code)}" id="student_name_sort">원생 이름순 정렬👉</th>
                <th class="col-2" onclick="${sort_consultingoption('deadline_desc',done_code)}" id="deadline_sort">마감일 정렬👉</th>
                <th class="col-2" onclick="${sort_consultingoption('consulting_desc',done_code)}" id="consulting_sort">상담 건 정렬👉</th>
            </tr>
            <tr class="row">
                <th class="col-2">반</th>
        ...
        `)
        sort_consultingoption('deadline_desc',done_code)
    }
}
...
async function get_consulting_history() {
    ...
    const target_list = myConsultingsData.filter(c => c.done == 1)
...
async function get_consulting_history_detail(c_id) {
    ...
    let temp_his = ''
    let consulting_history = myConsultingsData.filter(c => c.id == c_id)[0]
...
async function get_question_detail(q_id) {
    ...
    if(questiondata.student_id != 0){
        ban_student_data = myStudentData.filter(s => s.register_no == questiondata.student_id)[0]
        temp_question_list+=`
        <div class="modal-body-select-container">
    ...
    if (questiondata.category == 0 || questiondata.category == 4 || questiondata.category == 5) {
        ...
        consulting_history = myConsultingsData.filter(c => c.id == questiondata.consluting)[0]
        let category = ''
        if (consulting_history.category_id < 100) {
