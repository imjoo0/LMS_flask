...
// 전체 반 정보(차트) 가져오는 함수 
async function get_students_data() {
    try {
        if (!studentsData) {
            ...
            const message = new Promise((resolve, reject) => {
                ...
                studentsWorker.onerror = function (error) {
                    reject(error);
                };
            });
            studentsWorker.postMessage('getStudentsdata');
            studentsData = await message;
        }
    } catch (error) {
        alert('Error occurred while retrieving data.');
    }
...
async function get_ban_info(b_id) {
    ...
    BanChart = Chart.getChart('total-chart-element')
    if (BanChart) {
        BanChart.destroy()
    }
    ...
    if(!consultingData){
        ...
        function fetchData(b_id){
            ...
            consultingbanChunkWorker.postMessage({pageSize,b_id})
        }
