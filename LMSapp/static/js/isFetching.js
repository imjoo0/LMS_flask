...
self.onmessage = async function () {
  if (!isFetching) { // isFetching == false 일때 
    try {
      isFetching = true;
      const response = await fetch('/teacher/get_mybans');
      const data = await response.json();
      ban_data = data.ban_data;
      all_consulting = data.all_consulting;
      my_students = data.my_students;
      all_task = data.all_task;
    } catch (error) {
      alert('Error occurred while retrieving data.');
    } finally {
      isFetching = false;
      self.postMessage('data_fetched');
    }
  }
};
