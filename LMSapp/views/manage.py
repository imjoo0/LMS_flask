    ...
    if request.method == 'GET':
        ...
        try:
            with db.cursor() as cur:
                if(done_code == 0):
                    cur.execute('select id, title, contents, answer_id, create_date from question where answer_id IS NULL;')
                else:
                    cur.execute('select id, title, contents, answer_id, create_date from question where answer_id IS NOT NULL;')
