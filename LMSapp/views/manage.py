    ...
    if request.method == 'GET':
        ...
        try:
            with db.cursor() as cur:
                if(done_code == 0):
                    cur.execute('select id, title, contents, answer from question where answer IS NULL;')
                else:
                    cur.execute('select id, title, contents, answer from question where answer IS NOT NULL;')
