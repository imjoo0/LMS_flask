    ...
    if len(ban_data) != 0:
        ...
        try:
            with db.cursor() as cur:
                ...
                # 업무
                cur.execute("select taskban.id,taskban.ban_id, taskcategory.name as category, task.contents, task.deadline,task.priority,taskban.done,taskban.created_at from taskban left join task on taskban.task_id = task.id left join taskcategory on task.category_id = taskcategory.id where ( (task.category_id = 11) or ( (task.cycle = %s and taskban.created_at is null ) or (task.cycle = %s and taskban.created_at = curdate()) or (task.cycle = 0) ) ) and ( task.startdate <= %s and %s <= task.deadline ) and taskban.teacher_id=%s;", (today_yoil,today_yoil, Today, Today,ban_data[0]['id'],))
