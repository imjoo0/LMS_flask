    ...
    if request.method == 'GET':
        ...
        if len(mybans_info) != 0:
            ...
            try:
                with db.cursor() as cur:
                    # 상담
                    cur.execute("select id, student_id, category_id , done, date_format(deadline, '%Y-%m-%d') as deadline from consulting where startdate <= %s and teacher_id=%s;",(Today,session['user_registerno'],))
...
def mystudents(is_done):
    if request.method == 'GET':
        ...
        try:
            with db.cursor() as cur:
                cur.execute("select consulting.id, consulting.ban_id, consulting.student_id, consulting.contents, consulting.week_code, consulting.category_id, date_format(consulting.deadline, '%Y-%m-%d') as deadline, consultingcategory.name as category from consulting left join consultingcategory on consultingcategory.id = consulting.category_id where consulting.done=%s and consulting.teacher_id=%s;",(is_done,session['user_registerno']))
                all_consulting['status'] = 200
                all_consulting['data'] = cur.fetchall()
        except Exception as e:
            print(e)
