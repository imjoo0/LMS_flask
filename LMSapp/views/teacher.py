    ...
    if request.method == 'GET':
        ...
        if len(ban_data) != 0:
            ...
            try:
                with db.cursor() as cur:
                    # 상담
                    cur.execute("select id, student_id, category_id , done, deadline,missed,created_at from consulting where startdate <= %s and teacher_id=%s", (Today, session['user_registerno'],))
