        ...
        if len(mybans_info) != 0:
            ...
            try:
                with db.cursor() as cur:
                    ...
                    # 업무
                    cur.execute("select taskban.id,taskban.done from taskban left join task on taskban.task_id = task.id where task.startdate <= %s and %s <= task.deadline and taskban.teacher_id=%s;",(Today,Today,session['user_registerno'],))
                    all_task['status'] = 200
